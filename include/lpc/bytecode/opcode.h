...
enum class Op : std::uint8_t {
    ...
    // Jump (unconditional)
    // Encoding: [opcode:u8] [target_offset:i16]
    // target_offset is relative to the next instruction.
    // Stack: unchanged
    // Size: 3 bytes
    Jump = 34,
    ...
    // Encoding: [opcode:u8] [iter_local:u16] [jump_end:i16]
    // Pops container. Stores iterator state in iter_local.
    // If container is empty, jumps by jump_end from the next instruction.
    ...
    // 中文说明：下面几条是面向热点路径的 superinstruction。
    // 它们只改变编码和调度开销，不改变 LPC 语言语义；调试行表仍指向合成前的源行。

    // Increment/decrement a local variable in place.
    // Encoding: [opcode:u8] [local_index:u16]
    // Stack: unchanged
    IncLocal = 57,
    DecLocal = 58,

    // Superinstruction: if (!(locals[lhs] < locals[rhs])) jump.
    // 常见于 while (i < n) 这样的循环条件，避免两次 LoadLocal、一次 Lt 和一次条件跳转。
    // Encoding: [opcode:u8] [lhs_local:u16] [rhs_local:u16] [target_offset:i16]
    // target_offset is relative to the next instruction.
    // Stack: unchanged
    JumpIfLocalLtFalse = 59,

    // Superinstruction: locals[dest] = locals[lhs] + locals[rhs].
    // 常见于 sum = sum + i，避免局部变量反复进出操作数栈。
    // Encoding: [opcode:u8] [dest_local:u16] [lhs_local:u16] [rhs_local:u16]
    // Stack: unchanged
    AddLocalLocalToLocal = 60,

    // Superinstruction: ++locals[local]; jump.
    // 常见于循环尾部的 i++ 后立刻回跳。
    // Encoding: [opcode:u8] [local_index:u16] [target_offset:i16]
    // target_offset is relative to the next instruction.
    // Stack: unchanged
    IncLocalAndJump = 61,

    // Superinstruction: if (!(locals[lhs] <= iconst[rhs])) jump.
    // 常见于递归边界判断，例如 if (n <= 1)。
    // Encoding: [opcode:u8] [lhs_local:u16] [iconst_index:u16] [target_offset:i16]
    // target_offset is relative to the next instruction.
    // Stack: unchanged
    JumpIfLocalIConstLteFalse = 62,

    // Superinstructions for loading a computed local expression.
    // 常见于 fib(n - 1) / fib(n - 2) 这类调用参数构造。
    // Encoding: [opcode:u8] [local_index:u16]
    // Stack: [...] -> [..., locals[local] - 1]
    LoadLocalDec = 63,

    // Encoding: [opcode:u8] [local_index:u16] [iconst_index:u16]
    // Stack: [...] -> [..., locals[local] - iconst[rhs]]
    LoadLocalSubIConst = 64,

    // Encoding: [opcode:u8] [local_index:u16] [iconst_index:u16]
    // Stack: [...] -> [..., locals[local] + iconst[rhs]]
    // 常见于数组/mapping 字面量里的 i + 1、i + 100 等临时表达式。
    LoadLocalAddIConst = 65,

    // Encoding: [opcode:u8] [dest_local:u16] [lhs_local:u16] [container_local:u16] [iconst_key:u16]
    // Stack: unchanged
    // 语义等价于 locals[dest] = locals[lhs] + locals[container][iconst[key]]。
    AddLocalIndexIConstToLocal = 66,

    // Encoding: [opcode:u8] [dest_local:u16] [lhs_local:u16] [container_local:u16] [key_local:u16]
    // Stack: unchanged
    // 语义等价于 locals[dest] = locals[lhs] + locals[container][locals[key]]。
    AddLocalIndexLocalToLocal = 67,

    // Encoding: [opcode:u8] [dest_local:u16] [lhs_local:u16] [rhs_local:u16]
    //           [inc_local:u16] [cmp_lhs_local:u16] [cmp_rhs_local:u16] [target_offset:i16]
    // Stack: unchanged
    // 语义等价于：
    //   locals[dest] = locals[lhs] + locals[rhs];
    //   ++locals[inc];
    //   if (locals[cmp_lhs] < locals[cmp_rhs]) jump;
    // 常见于 while (i < n) { sum = sum + i; i = i + 1; } 的循环尾部。
    AddLocalLocalIncJumpIfLocalLt = 68,

    // Encoding: [opcode:u8] [dest_local:u16] [lhs_local:u16] [iconst_index:u16]
    // Stack: unchanged
    // 语义等价于 locals[dest] = locals[lhs] + iconst[rhs]。
    // 常见于 hp = hp + 10、exp = exp + reward 这类业务计数更新。
    AddLocalIConstToLocal = 69,
