...
static void WriteU64(std::vector<char> &buf, std::uint64_t v) {
    for (int i = 0; i < 8; ++i) {
...
static bool LowerMirToNextVm(
    ...
    auto ensure_iconst = [&](std::int64_t literal) {
        const int key = static_cast<int>(literal);
        auto it = iconst_index.find(key);
        if (it != iconst_index.end()) {
            return it->second;
        }
        int idx = static_cast<int>(iconsts.size());
        iconst_index[key] = idx;
        iconsts.push_back(literal);
        return idx;
    };
    auto emit_load_iconst = [&](std::int64_t literal) {
        int idx = ensure_iconst(literal);
        emit_u8(static_cast<std::uint8_t>(NextOp::LoadIConst));
    ...
    for (int i = 0; i < static_cast<int>(fn.code.size()); ++i) {
        ...
        case MirOp::LoadFunction:
            emit_load_iconst(mi.a);
            break;
        case MirOp::LoadLocal:
            emit_u8(static_cast<std::uint8_t>(NextOp::LoadLocal));
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        case MirOp::StoreLocal:
            emit_u8(static_cast<std::uint8_t>(NextOp::StoreLocal));
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        case MirOp::Add:
            emit_u8(static_cast<std::uint8_t>(NextOp::Add));
            break;
        case MirOp::Sub:
            emit_u8(static_cast<std::uint8_t>(NextOp::Sub));
            break;
        case MirOp::Mul:
            emit_u8(static_cast<std::uint8_t>(NextOp::Mul));
            break;
        case MirOp::Div:
            emit_u8(static_cast<std::uint8_t>(NextOp::Div));
            break;
        case MirOp::Mod:
            emit_u8(static_cast<std::uint8_t>(NextOp::Mod));
            break;
        case MirOp::LogicAnd:
            emit_u8(static_cast<std::uint8_t>(NextOp::LogicAnd));
            break;
        case MirOp::LogicOr:
            emit_u8(static_cast<std::uint8_t>(NextOp::LogicOr));
            break;
        case MirOp::LogicNot:
            emit_u8(static_cast<std::uint8_t>(NextOp::LogicNot));
            break;
        case MirOp::Eq:
            emit_u8(static_cast<std::uint8_t>(NextOp::Eq));
            break;
        case MirOp::Neq:
            emit_u8(static_cast<std::uint8_t>(NextOp::Neq));
            break;
        case MirOp::Lt:
            emit_u8(static_cast<std::uint8_t>(NextOp::Lt));
            break;
        case MirOp::Lte:
            emit_u8(static_cast<std::uint8_t>(NextOp::Lte));
            break;
        case MirOp::Gt:
            emit_u8(static_cast<std::uint8_t>(NextOp::Gt));
            break;
        case MirOp::Gte:
            emit_u8(static_cast<std::uint8_t>(NextOp::Gte));
            break;
        case MirOp::Index:
            if (mi.a != 0) {
                return fail("nextvm lowering does not support slice index yet");
            }
            emit_u8(static_cast<std::uint8_t>(NextOp::Index));
            break;
        case MirOp::NewArray:
            if (mi.a < 0 || mi.a > 0xffff) {
                return fail("nextvm lowering array size out of range");
            }
            emit_u8(static_cast<std::uint8_t>(NextOp::NewArray));
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        case MirOp::NewClass:
            emit_u8(static_cast<std::uint8_t>(NextOp::NewClass));
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        case MirOp::LoadClassField:
            emit_u8(static_cast<std::uint8_t>(NextOp::LoadClassField));
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        case MirOp::StoreClassField:
            emit_u8(static_cast<std::uint8_t>(NextOp::SetClassField));
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        case MirOp::Call: {
            if (mi.a < 0 || out.size() < 3 ||
                out[out.size() - 3] != static_cast<std::uint8_t>(NextOp::LoadIConst)) {
                return fail("nextvm lowering only supports direct function calls for now");
            }
            ...
            if (cidx >= iconsts.size() || iconsts[cidx] < 0 || iconsts[cidx] > 0xffff) {
                return fail("nextvm call target constant out of range");
            }
            std::uint16_t fid = static_cast<std::uint16_t>(iconsts[cidx]);
            out.erase(out.end() - 3, out.end());
            emit_u8(static_cast<std::uint8_t>(NextOp::CallValue));
            emit_u16(fid);
            emit_u16(static_cast<std::uint16_t>(mi.a));
            break;
        }
        case MirOp::JumpIfFalse: {
            emit_u8(static_cast<std::uint8_t>(NextOp::JumpIfFalse));
            int patch_pos = static_cast<int>(out.size());
            emit_u16(0);
        ...
        case MirOp::Jump: {
            emit_u8(static_cast<std::uint8_t>(NextOp::Jump));
            int patch_pos = static_cast<int>(out.size());
            emit_u16(0);
            patches.push_back({patch_pos, mi.a});
            break;
        ...
        case MirOp::Return:
            emit_u8(static_cast<std::uint8_t>(NextOp::Return));
            break;
        case MirOp::Pop:
            emit_u8(static_cast<std::uint8_t>(NextOp::Pop));
            break;
        case MirOp::Dup:
            emit_u8(static_cast<std::uint8_t>(NextOp::Dup));
            break;
        default:
...
bool WriteMirAsNextVmBytecode(const MirModule &module, const std::string &module_name, const std::string &out_path, std::string *error) {
    ...
    for (const auto &fn : module.functions) {
        ...
        std::vector<int> mir_pc_to_byte;
        if (!LowerMirToNextVm(fn, iconst_index, iconsts, code, error, &mir_pc_to_byte)) {
    ...
    write_bytes("LPCNVM1", 7);
    WriteU8(all, 0);
    write_string(module_name);

    WriteU32(all, static_cast<std::uint32_t>(iconsts.size()));
    for (std::int64_t v : iconsts) {
        WriteU64(all, static_cast<std::uint64_t>(v));
    }
    ...
    WriteU32(all, static_cast<std::uint32_t>(class_names.size()));
    for (const auto &name : class_names) {
        auto it = module.class_fields.find(name);
        std::uint16_t nfields = it == module.class_fields.end()
            ? 0
            : static_cast<std::uint16_t>(it->second.size());
        WriteU16(all, nfields);
    }

    WriteU32(all, static_cast<std::uint32_t>(lowered.size()));
    for (const auto &lf : lowered) {
        write_string(lf.name);
        WriteU16(all, static_cast<std::uint16_t>(lf.nargs));
        WriteU16(all, static_cast<std::uint16_t>(lf.nlocals));
        WriteU16(all, static_cast<std::uint16_t>(lf.max_stack));
        WriteU32(all, static_cast<std::uint32_t>(lf.from));
        WriteU32(all, static_cast<std::uint32_t>(lf.to));
    }
    ...
    WriteU32(all, static_cast<std::uint32_t>(line_map.size()));
    for (const auto &it : line_map) {
        WriteU32(all, it.first);
        WriteU32(all, it.second);
    }

    WriteU32(all, static_cast<std::uint32_t>(code.size()));
    for (std::uint8_t b : code) {
        WriteU8(all, b);
    }
    ...
    if (!out.is_open()) {
        if (error) {
            *error = "failed to open output nextvm bytecode file";
        }
        return false;
    }
    out.write(all.data(), static_cast<std::streamsize>(all.size()));
...
bool WriteMirModuleSetAsNextVmBytecodeAtPath(
    ...
    if (!WriteMirAsNextVmBytecode(module, module_name, out_file.string(), error)) {
        return false;
    }
