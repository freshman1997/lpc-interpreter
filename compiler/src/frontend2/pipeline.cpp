...
#include "frontend2/lexer.h"
#include "frontend2/parser2.h"
#include "frontend2/preprocessor.h"
    ...
    void EmitExpr(const Expr *expr, MirFunction &f) {
        ...
        case NodeKind::CallExpr: {
            ...
            if (c->callee && c->callee->kind == NodeKind::Identifier) {
                ...
                int eidx = Sema2::EfunIndex(id->name);
                if (eidx >= 0) {
...
PipelineResult CompileSourceToMir(
    ...
    Parser2 parser(&result.diagnostics);
    std::unique_ptr<Module> module = parser.Parse(tokens);

    if (module) {
        Sema2 sema(&result.diagnostics);
