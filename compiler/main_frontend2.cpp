...
static void DumpMirFunction(std::ostream &os, const lpc::frontend2::MirFunction &f, const std::string &title) {
    os << "fn " << f.name << " -- " << title << "\n";
...
static void DumpMirModule(const lpc::frontend2::MirModule &m, const std::string &path, const std::string &title) {
...
static int compile_one(const std::string &path, const CompileOptions &opt) {
    ...
    buf << in.rdbuf();
    lpc::frontend2::PipelineOptions popt;
    popt.keep_pre_opt_module = opt.dump_mir_before;
    lpc::frontend2::PipelineResult result = lpc::frontend2::CompileSourceToMir(path, buf.str(), opt.include_dirs, popt);

    const auto &diags = result.diagnostics.All();
    for (const auto &d : diags) {
        const char *lvl = d.level == lpc::frontend2::DiagnosticLevel::Error ? "error" :
                          d.level == lpc::frontend2::DiagnosticLevel::Warning ? "warning" : "note";
    ...
    if (!lpc::frontend2::WriteMirModuleSetAsV1BytecodeAtPath(result.module, path, opt.workspace_root, opt.out_root, &out_err, &out_module)) {
    ...
    std::cout << "frontend2 ok: " << path << ", functions=" << result.module.functions.size() << "\n";
...
int main(int argc, char **argv) {
    ...
    if (std::string(argv[1]) == "--test") {
        int failed = lpc::frontend2::RunGoldenTests();
        if (failed == 0) {
            std::cout << "frontend2 golden tests: PASS\n";
            return 0;
        }
        std::cout << "frontend2 golden tests: FAIL count=" << failed << "\n";
        return 5;
    }
