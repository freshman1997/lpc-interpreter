...
static lpc::core::Status TranslateV1ToVm2(
    ...
    std::unordered_map<std::string, std::uint16_t> func_name_to_vm2;
    for (std::uint16_t i = 0; i < static_cast<std::uint16_t>(v1_funcs.size()); ++i) {
        func_name_to_vm2[v1_funcs[i].name] = i;
    }
    ...
    for (int fi = 0; fi < static_cast<int>(v1_funcs.size()); ++fi) {
        ...
        while (ip < vf.to) {
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_load_iconst)) {
                ...
                ip += 2;
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::LoadIConst));
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_load_local) || op == static_cast<std::uint8_t>(OpCode::op_store_local)) {
                ...
                out->code.push_back(static_cast<std::uint8_t>(
                    op == static_cast<std::uint8_t>(OpCode::op_load_local)
                        ? lpc::vm2::Op::LoadLocal
                        : lpc::vm2::Op::StoreLocal));
                EmitU16(&out->code, idx);
                continue;
            }
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_add)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Add));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_sub)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Sub));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_mul)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Mul));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_div)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Div));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_mod)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Mod));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_eq)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Eq));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_neq)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Neq));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_gt)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Gt));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_gte)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Gte));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_lt)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Lt));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_lte)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Lte));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_and)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::LogicAnd));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_or)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::LogicOr));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_cmp_not)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::LogicNot));
                continue;
            }
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_new_class)) {
                ...
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::NewClass));
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_set_class_field)) {
                ...
                std::uint16_t field_idx = static_cast<std::uint16_t>(v1_code[ip]) |
                    static_cast<std::uint16_t>(v1_code[ip + 1] << 8);
                ip += 2;
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::SetClassField));
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_load_class_field)) {
                ...
                std::uint16_t field_idx = static_cast<std::uint16_t>(v1_code[ip]) |
                    static_cast<std::uint16_t>(v1_code[ip + 1] << 8);
                ip += 2;
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::LoadClassField));
                EmitU16(&out->code, field_idx);
                continue;
            }
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_new_array)) {
                ...
                ip += 4;
                if (n > 0xffff) {
                    return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, "new_array too large for vm2 operand");
                }
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::NewArray));
                EmitU16(&out->code, static_cast<std::uint16_t>(n));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_index)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Index));
                continue;
            }

            if (op == static_cast<std::uint8_t>(OpCode::op_return)) {
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::Return));
                has_return = true;
                continue;
            }
            ...
            if (op == static_cast<std::uint8_t>(OpCode::op_call)) {
                ...
                if (type != 0) {
                    ...
                    return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, "vm2 bridge only supports local call type=0");
                }

                if (out->code.size() < 3 || out->code[out->code.size() - 3] != static_cast<std::uint8_t>(lpc::vm2::Op::LoadIConst)) {
                    return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, "call without preceding callee const in bridge");
                }
                ...
                if (!func_name_to_vm2.count(callee_name)) {
                    return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, "call callee function name not found in vm2 map");
                }
                std::uint16_t mapped_callee = func_name_to_vm2[callee_name];

                if (mapped_callee == static_cast<std::uint16_t>(fi)) {
                    std::string msg = "self-recursive call is not supported in vm2 bridge yet: caller=" +
                ...
                out->code.push_back(static_cast<std::uint8_t>(lpc::vm2::Op::CallValue));
            ...
            std::string msg = "unsupported v1 opcode in vm2 bridge: " + std::to_string(static_cast<int>(op));
            return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, msg);
        }
        ...
        for (const Patch &p : patches) {
            if (!old_to_new.count(p.target_old)) {
                return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, "jump target not aligned in bridge");
            }
            ...
            if (rel < -32768 || rel > 32767) {
                return lpc::core::Status::Error(lpc::core::ErrorCode::VmError, "jump target too far for vm2 rel16");
            }
...
core::Status RunEntryModuleVm2(const std::string &entry_module) {
    ...
    if (path.empty()) {
        return core::Status::Error(core::ErrorCode::NotFound, "vm2 could not find module bytecode");
    }
    ...
    vm2::Chunk ch;
    s = TranslateV1ToVm2(module_name, funcs, iconst, code, &ch);
    if (!s.ok()) {
        return s;
    }

    vm2::Vm vm2_engine;
    vm2::RuntimeError e = vm2_engine.LoadChunk(ch);
    if (!e.ok()) {
        return core::Status::Error(core::ErrorCode::VmError, "vm2 load chunk failed: " + e.message);
    }
    e = vm2_engine.RunEntry("main");
    if (!e.ok()) {
        return core::Status::Error(core::ErrorCode::VmError, "vm2 run failed: " + e.message);
    }

    vm2::Value out = vm2_engine.last_result();
    if (out.tag == vm2::ValueTag::Int64) {
        std::cout << "vm2 result: " << out.as.i64 << std::endl;
    }
    return core::Status::OkStatus();
}

core::Status RunEntryModuleCompare(const std::string &entry_module) {
    std::cout << "[compare] protocol: vm2-first, legacy-inprocess-best-effort" << std::endl;
    ...
    vm2::RuntimeError e = vm2_engine.LoadChunk(ch);
    if (!e.ok()) {
        std::cout << "[compare] vm2_status=error" << std::endl;
        return core::Status::Error(core::ErrorCode::VmError, "vm2 load chunk failed: " + e.message);
    }
    e = vm2_engine.RunEntry("main");
    if (!e.ok()) {
    ...
    std::cout << "[compare] vm2_status=ok" << std::endl;
    ...
    vm2::Value v2 = vm2_engine.last_result();
    if (v2.tag == vm2::ValueTag::Int64 && legacy->has_last_int_result()) {
        std::cout << "[compare] vm2_result_int=" << v2.as.i64 << std::endl;
        ...
        if (static_cast<lint64_t>(legacy->last_int_result()) != v2.as.i64) {
            return core::Status::Error(core::ErrorCode::VmError, "result mismatch between vm2 and legacy");
        }
