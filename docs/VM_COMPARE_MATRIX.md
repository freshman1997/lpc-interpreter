...
This file tracks modules that are expected to pass VM1 vs VM2 compare in current stage.
...
- `compiler/tests/golden/sample_vm2_compare_index_mul`
...
- efun-rich modules (beyond current minimal bridge coverage)

## Known note

- (resolved) previous legacy memory pressure note for `sample_vm2_compare_class_field` should be fully removed once compare matrix runs all-pass without note fallback.
